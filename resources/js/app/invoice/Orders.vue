<template>
  <div class="app-container">
    <div v-if="page.option==='list'">
      <router-link
        v-if="checkPermission(['create invoice']) && canCreateNewOrder"
        :to="{name:'CreateOrder'}"
        ...
        <v-client-table v-model="invoices" :columns="columns" :options="options">
          ...
          >
            <div v-if="props.row.waybill_items.length > 0">
              ...
              <div v-else class="label label-warning">
                <router-link v-if="checkPermission(['generate waybill'])" :to="{name:'GenerateWaybill'}"> Partially</router-link>
              </div>
            </div>
            <div v-else class="alert alert-danger">
              <router-link v-if="checkPermission(['generate waybill'])" :to="{name:'GenerateWaybill'}"> No</router-link>
            </div>
          </div>
          <div
            slot="invoice_date"
          ...
          <div slot="action" slot-scope="props">
            ...
            <a
              ...
              @click="deleteOrder(props.index, props.row)"
            >
              <i class="fa fa-trash" />
            </a>
    ...
    <div v-if="page.option==='edit_invoice'">
      <a class="btn btn-danger no-print" @click="page.option='list'">Go Back</a>
      <edit-order :invoice="invoice" :page="page" :params="params" @update="onEditUpdate" />
    </div>
  </div>
</template>
<script>
...
const fetchOrders = new Resource('invoice/general');
// const approveOrderResource = new Resource('invoice/general/approve');
// const deliverOrderResource = new Resource('invoice/general/deliver');
const cancelOrderResource = new Resource('invoice/general/cancel');
const deleteOrderResource = new Resource('invoice/general/delete');
export default {
  components: { OrderDetails, EditOrder, Pagination },
  props: {
    canCreateNewOrder: {
      type: Boolean,
      default: () => true,
  ...
  methods: {
    ...
    cancelOrder(index, invoice) {
      const app = this;
      const param = { status: 'cancelled' };
      cancelOrderResource.update(invoice.id, param).then(response => {
        app.invoices.splice(index - 1, 1);
      });
    },
    deleteOrder(index, invoice) {
      const app = this;
      const message = 'Are you sure? This cannot be undone!';
      if (confirm(message)) {
        deleteOrderResource.destroy(invoice.id, invoice).then(response => {
          app.invoices.splice(index - 1, 1);
        });
      }
    },
